    ...
    const handleJson = (json) => {
      const list = (json?.data ?? []).map((p) => {
        ...
        return {
          ...
          id: KEY_TO_ID[key] ?? 1,
          label:
            p.name ?? KEY_TO_LABEL[key] ?? clusterByKey[key]?.persona ?? key,
        };
      });
      _personaCache = list;
      setPersonas(list);
                ...
                >
                  {row.map((item) => (
                    <div
                      ...
                      style={{
                        flex: `0 1 ${row.length === 1 ? "400px" : row.length === 2 ? "calc(50% - 1rem)" : "calc(33% - 1rem)"}`,
                        maxWidth: row.length === 1 ? "450px" : "450px",
                      }}
                    >
                      <img
