...
| POST   | `/chat`           | Chat / Admin (พิมข้อความ) |
| POST   | `/chat/quick`     | Chat / Admin (Quick Prompts) |

---

## 🛠️ สิ่งที่ปรับปรุงล่าสุด

- **Admin — Quick Chat → `/chat/quick`**: ปุ่ม Quick Prompts ในหน้า Admin จะ POST ไป `/chat/quick` พร้อม payload `{ cluster, prompt, custer, attibute }` โดย `attibute` แปลงค่าดาวเป็นข้อความ criteria อัตโนมัติ
- **Admin — Regular Chat → `/chat`**: พิมข้อความเองจะส่ง `{ personId, prompt }` ไป `/chat`
- **Admin — เปลี่ยน Persona ล้างแชท**: เลือก persona ใหม่จาก dropdown แชทจะ reset กลับเป็นข้อความต้อนรับใหม่
- **Admin — Debounce `/persona/save`**: หลังแก้ค่า attribute หรือ Person ID จะรอ 2.5 วินาทีหลังหยุดพิม แล้วค่อย save ทีเดียว (ลดการเรียก API ซ้ำ)
- **MainMenu — Cache `/all_persona`**: เรียก API แค่ครั้งแรก ครั้งต่อไปใช้ค่า cache ในหน่วยความจำ (reset เมื่อ reload)
- **ถอด `<React.StrictMode>`**: แก้ปัญหา `useEffect` ทำงาน 2 รอบในโหมด dev ทำให้ทุก API ถูกเรียกครั้งเดียวตามปกติ
